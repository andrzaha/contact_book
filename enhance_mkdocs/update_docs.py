...
def generate_markdown(script_content):
    """
...
def update_mkdocs_yml(markdown_file):
    ...
    doc_entry = {os.path.basename(markdown_file).replace(".md", ""): os.path.basename(markdown_file)}
...
def main(script_path, docs_dir):
    ...
    markdown_content = generate_markdown(script_content)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate documentation for a Python script.")
    parser.add_argument("script_path", help="Path to the Python script")
    parser.add_argument("--docs_dir", default="../docs", help="Path to the docs directory (default: ../docs)")
